"""TGE RDN sensor platform - v1.7.3"""
import logging
import asyncio
import io
import re
...
async def async_setup_entry(
    ...
    if coordinator.data:
        ...
        _LOGGER.info(f"âœ… TGE RDN Integration ready! Today: {'âœ…' if today_available else 'âŒ'}, Tomorrow: {'âœ…' if tomorrow_available else 'âŒ'}")
    else:
...
class TGERDNDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def hourly_update_callback(self, now: datetime) -> None:
        ...
        if self.last_hour_updated != current_hour:
            _LOGGER.info(f"â° Hour boundary detected: {self.last_hour_updated}:XX â†’ {current_hour}:XX - forcing update")
        ...
        # Also update at specific aligned times
            ...
            _LOGGER.debug(f"â° Aligned time update: {current_hour}:{current_minute:02d}")
    ...
    def _find_excel_url_for_date(self, target_date):
        ...
        for link in soup.find_all('a', href=True):
            href = link['href']
            if href.endswith('.xlsx') and date_str in href and 'Raport_RDN' in href:
    ...
    async def async_config_entry_first_refresh(self) -> None:
        ...
        try:
            ...
            if data:
                ...
                today_status = "âœ… Available" if data.get("today") else "âŒ Not available"  
                tomorrow_status = "âœ… Available" if data.get("tomorrow") else "âŒ Not available"
            ...
            _LOGGER.error(f"âŒ Error during immediate fetch: {err}")
    ...
    async def _force_fetch_both_days(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if tomorrow_data:
                ...
                _LOGGER.info(f"ðŸ“… FORCE FETCH: Tomorrow's data ({tomorrow.strftime('%A')}) not available yet")
                self.tomorrow_data_available = False
            ...
            _LOGGER.error(f"âŒ FORCE FETCH failed: {err}")
            return {}
    ...
    async def _async_update_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.last_hour_updated != now.hour:
                update_reason = f"hour_change ({self.last_hour_updated} â†’ {now.hour})"
                self.last_hour_updated = now.hour

            _LOGGER.info(f"ðŸ”„ Regular update cycle at {now.strftime('%H:%M:%S')} on {now.strftime('%A')} ({update_reason})")
            ...
            # Log summary
            today_status = "âœ… Available" if today_data else "âŒ Not available"
            tomorrow_status = "âœ… Available" if tomorrow_data else "âŒ Not available"
            _LOGGER.info(f"âœ… Regular update complete: Today {today_status}, Tomorrow {tomorrow_status}")

            return result

        except Exception as err:
            _LOGGER.error(f"âŒ Error in regular update: {err}")
        ...
        if should_fetch_tomorrow:
            ...
            if new_tomorrow_data:
                ...
                if self.data and self.data.get("tomorrow"):
                    _LOGGER.debug(f"ðŸ“… Failed to fetch new tomorrow data, preserving existing")
            ...
            if self.data and self.data.get("tomorrow"):
                _LOGGER.debug("ðŸ“… Preserving existing tomorrow data (outside fetch window)")
                return self.data.get("tomorrow")
            else:
    ...
    async def _fetch_day_data(self, date: datetime, day_type: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            _LOGGER.error(f"âŒ Unexpected error fetching {day_type} data ({date.strftime('%A')}): {err}")
            return None
    ...
    def _parse_excel_data(self, file_content: bytes, date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.error(f"âŒ Unexpected error parsing Excel data for {date.date()}: {err}")
            raise
...
class TGERDNSensor(CoordinatorEntity, SensorEntity):
    ...
    def state(self) -> Optional[float]:
        ...
        try:
            ...
            if (self._sensor_type == "current_price" and 
                ...
                _LOGGER.info(f"â° Current price sensor: Hour changed {self._last_state_hour} â†’ {current_hour}")
    ...
    def _get_distribution_rate(self, when) -> float:
        ...
        if is_weekend or is_polish_holiday:
            if is_polish_holiday:
                _LOGGER.debug(f"ðŸ‡µðŸ‡± Polish holiday detected: {local.date()} - using lowest distribution rate")
    ...
    def _is_polish_holiday(self, date) -> bool:
        ...
        fixed_holidays = [
            ...
            (1, 6),   # Epiphany - ÅšwiÄ™to Trzech KrÃ³li
            (5, 1),   # May Day - ÅšwiÄ™to Pracy
            (5, 3),   # Constitution Day - ÅšwiÄ™to Konstytucji 3 Maja
            (8, 15),  # Assumption Day - WniebowziÄ™cie NMP
            (11, 1),  # All Saints Day - Wszystkich ÅšwiÄ™tych
            (11, 11), # Independence Day - ÅšwiÄ™to NiepodlegÅ‚oÅ›ci
            (12, 25), # Christmas Day - BoÅ¼e Narodzenie
            (12, 26), # Boxing Day - Drugi DzieÅ„ ÅšwiÄ…t BoÅ¼ego Narodzenia
        ]
        ...
        moveable_holidays = [
            ...
            easter_date + timedelta(days=1),   # Easter Monday - PoniedziaÅ‚ek Wielkanocny
            easter_date + timedelta(days=49),  # Whit Sunday - Zielone ÅšwiÄ…tki
            easter_date + timedelta(days=60),  # Corpus Christi - BoÅ¼e CiaÅ‚o
        ]
    ...
    def _compute_total_price(self, base_pln_mwh: float, when, debug_info: bool = False) -> Dict[str, Any]:
        ...
        if debug_info or is_negative or is_polish_holiday:
            ...
            if is_polish_holiday:
                _LOGGER.debug(f"ðŸ‡µðŸ‡± Polish holiday pricing: Distribution={dist_rate:.2f} PLN/MWh (lowest rate)")
